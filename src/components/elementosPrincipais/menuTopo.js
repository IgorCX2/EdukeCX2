    ...
    return (
        <header className={`${paramsLink == undefined && 'text-white'} absolute w-full top-0 z-20`}>
            ...
            <Container configuracao="flex justify-between items-center py-5">
                ...
                <div className={`lg:hidden flex flex-col gap-1.5 cursor-pointer ${menu == true && 'hidden'}`}onClick={() => setMenu(!menu)}>
                    <span className={`w-8 h-0.5 ${paramsLink == undefined ? 'bg-white' : 'bg-black'}`}/>
                    <span className={`w-8 h-0.5 ${paramsLink == undefined ? 'bg-white' : 'bg-black'}`}/>
                    <span className={`w-8 h-0.5 ${paramsLink == undefined ? 'bg-white' : 'bg-black'}`}/>
                </div>
                <div className={`fixed lg:relative ${menu == true ? 'flex text-black z-30' : 'hidden'} p-6 lg:p-0 top-0 w-80 lg:w-full lg:items-center max-w-[calc(100%-3rem)] lg:max-w-none flex-col xl:flex-row lg:flex left-0 h-full lg:h-auto gap-5 xl:gap-0 justify-between bg-gray-200 lg:bg-transparent`} onClick={() => setMenu(false)}>
